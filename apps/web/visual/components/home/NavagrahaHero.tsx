    ...
    <section className="relative min-h-screen" style={{ background: "#0A0608", overflow: "hidden" }}>
      ...
      <div className="relative z-20 flex flex-col lg:flex-row w-full min-h-[100dvh]">
        <div className="w-full lg:w-[55%] flex-shrink-0 flex flex-col justify-center text-center lg:text-left px-6 lg:pl-16 xl:pl-24 lg:pr-14 pt-20 pb-14 lg:pt-24 lg:pb-0 flex-1">
          ...
          {/* Dropped "With" and lowered the minimum size: at clamp's old 2.4rem
              floor the old wording ran to four lines on a phone. Two now. */}
          <h1 className="mb-6" style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "clamp(1.95rem,5vw,4.2rem)", fontWeight: 700, color: IVORY, lineHeight: 1.1, letterSpacing: "-0.02em" }}>
            <span style={{ color: "#FFFFFF" }}>{t("hero.title1", "Invoke Divine Energy")}</span><br />
            <span style={{ color: SAFFRON }}>{t("hero.title2", "Sacred Vedic Products")}</span>
          </h1>
          {/* The descriptive paragraph that sat here is gone — the eyebrow line
              above is the only subheading now. */}
          <div className="flex flex-wrap gap-3 justify-center lg:justify-start mb-8">
            ...
            <button onClick={onConsult} className="flex items-center gap-2 px-7 py-3.5 rounded-full text-sm font-semibold tracking-wide border transition-all hover:bg-white/8 whitespace-nowrap"
              ...
              {t("hero.cta_consult", "Consult Now")}
            </button>
          </div>
